...
@Component({
  ...
  template: `
          ...
          <tr *ngFor="let p of tb.data">
            ...
            <td>
              <div style="display:flex; gap:6px; align-items:center">
                ...
                <button nz-button nzSize="small" nzType="default" (click)="verCarnes(p)">
                  <span nz-icon nzType="book"></span> Carnês
                </button>
              </div>
            </td>
          </tr>
      ...
      <ng-container *ngIf="!carregando">
        ...
        <div *ngFor="let p of participantes" class="p-item">
          ...
          <div class="p-actions">
            ...
            <button class="p-btn primary" (click)="verCarnes(p)">
    ...
    <!-- Modal: carnês do participante -->
    <nz-modal
      [(nzVisible)]="carnesModalVisivel"
      [nzTitle]="'Carnês — ' + (carnesParticipante?.nome ?? '')"
      [nzFooter]="null"
      nzWidth="500px"
      (nzOnCancel)="carnesModalVisivel = false">
      <ng-container *nzModalContent>
        <nz-spin *ngIf="carregandoCarnes"
                 style="display:block; text-align:center; padding:32px" />

        <nz-empty *ngIf="!carregandoCarnes && carnes.length === 0"
                  nzNotFoundContent="Nenhum carnê nesta campanha" />

        <ng-container *ngIf="!carregandoCarnes && carnes.length > 0">
          <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:16px; flex-wrap:wrap; gap:8px">
            <span style="color:#888; font-size:13px">
              Campanha: <strong>{{ campanhaAtiva.campanha()?.nome }}</strong>
            </span>
            <div style="display:flex; gap:8px">
              <nz-tag nzColor="success">{{ carnesQuitados }} quitado(s)</nz-tag>
              <nz-tag>{{ carnes.length }} total</nz-tag>
            </div>
          </div>
          <div *ngFor="let c of carnes" class="carne-card">
            <div class="carne-header">
              <span class="carne-numero">Nº {{ c.numeroSorte }}</span>
...
export class ParticipantesListComponent implements OnInit, OnDestroy {
  ...
  carnesModalVisivel = false;
  carnesParticipante: any = null;
  carnes: any[] = [];
  ...
  ngOnInit() {
    ...
    this.buscaSub = this.buscaSubject.pipe(
      ...
      switchMap(busca => {
        ...
        if (this.somenteQuitados && campanhaId) {
          params['campanhaId'] = campanhaId;
          params['somenteQuitados'] = 'true';
        }
        return this.api.get<any[]>('participantes', Object.keys(params).length ? params : undefined);
      }),
    ).subscribe({
  ...
  carregar() {
    ...
    this.api.get<any[]>('participantes', Object.keys(params).length ? params : undefined).subscribe({
      next: (data) => { this.participantes = data; this.carregando = false; },
      error: () => { this.carregando = false; },
    });
  }
  ...
  verCarnes(participante: any) {
    this.carnesParticipante = participante;
    this.carnes = [];
    this.carregandoCarnes = true;
    this.carnesModalVisivel = true;
    const campanhaId = this.campanhaAtiva.campanha()?.id;
    this.api.get<any[]>('carnes/participante', { campanhaId, participanteId: participante.id }).subscribe({
      next: (data) => { this.carnes = data; this.carregandoCarnes = false; },
      error: () => { this.carregandoCarnes = false; },
    });
  }

  get carnesQuitados() { return this.carnes.filter(c => c.quitado).length; }
  parcelasPagas(c: any) { return c.parcelas.filter((p: any) => p.status === 'paga').length; }
  ...
  salvar() {
    ...
    if (this.editando) {
      const body: any = { nome: v.nome };
      if (v.telefone !== null) body.telefone = v.telefone;
      if (v.endereco !== null) body.endereco = v.endereco;
      this.api.patch<any>(`participantes/${this.editando.id}`, body).subscribe({
        next: () => {
      ...
      this.api.post<any>('participantes', body).subscribe({
        next: () => {
