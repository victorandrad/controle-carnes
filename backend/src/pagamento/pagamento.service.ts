...
export class PagamentoService {
  ...
  async registrar(dto: RegistrarPagamentoDto, usuarioId: string) {
    const parcela = await this.prisma.parcela.findUnique({
      ...
      include: { carne: { include: { campanha: true } } },
    });

    if (!parcela) throw new NotFoundException('Parcela não encontrada');
    if (parcela.status === 'paga') throw new BadRequestException('Parcela já está paga');
    if (parcela.carne.status !== 'ativo') {
      throw new BadRequestException('Carnê cancelado');
    }

    const dataPagamento = new Date(dto.dataPagamento);
    const agora = new Date();
    const diffDias = Math.floor((agora.getTime() - dataPagamento.getTime()) / 86_400_000);
    if (diffDias > 30) {
  ...
  listarPorCarne(carneId: string) {
    return this.prisma.pagamento.findMany({
      where: { parcela: { carneId } },
