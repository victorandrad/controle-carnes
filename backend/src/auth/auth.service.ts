...
export class AuthService {
  ...
  private readonly accessExpiry  = process.env.JWT_EXPIRES_IN          ?? '15m';
  private readonly refreshExpiry = process.env.JWT_REFRESH_EXPIRES_IN  ?? '30d';
  ...
  login(usuario: { id: string; email: string; role: string; nome: string }) {
    ...
    return {
      access_token:  this.jwtService.sign(payload, { secret: this.accessSecret, expiresIn: this.accessExpiry as any }),
      refresh_token: this.jwtService.sign({ sub: usuario.id }, { secret: this.refreshSecret, expiresIn: this.refreshExpiry as any }),
  ...
  async refresh(refreshToken: string) {
    try {
      ...
      return {
        access_token: this.jwtService.sign(newPayload, { secret: this.accessSecret, expiresIn: this.accessExpiry as any }),
      };
    } catch {
