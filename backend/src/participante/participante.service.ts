...
import { CriarParticipanteDto, AtualizarParticipanteDto } from './participante.dto';
...
export class ParticipanteService {
    ...
    if (temBusca || temFiltroQuitados) {
      ...
      if (temBusca) {
        ...
        if (buscoDigits.length > 0) {
          ...
          orParts.push(Prisma.sql`REGEXP_REPLACE(p.cpf, '[^0-9]', '', 'g') LIKE ${digitsLike}`);
        }
  ...
  async criar(dto: CriarParticipanteDto) {
    if (dto.cpf) {
      const existe = await this.prisma.participante.findUnique({ where: { cpf: dto.cpf } });
