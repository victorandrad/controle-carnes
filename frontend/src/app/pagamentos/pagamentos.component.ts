...
@Component({
  ...
  `],
  template: `
    <nz-page-header nzTitle="Pagamentos" nzSubtitle="Busque por nome, CPF ou telefone e selecione um participante" />
    ...
    <div class="root">
      ...
      <div class="painel-right" [class.tem-participante]="!!participanteId">
        ...
        <div class="mobile-stats" *ngIf="participanteId && !carregandoCarnes">
          <div class="ms-item">
            <div class="ms-val">{{ carnes.length }}</div>
            <div class="ms-lbl">Carnês</div>
          </div>
          <div class="ms-item">
            <div class="ms-val verde">{{ carnesQuitados }}</div>
            <div class="ms-lbl">Quitados</div>
          </div>
          <div class="ms-item">
            <div class="ms-val laranja">{{ carnes.length - carnesQuitados }}</div>
        ...
        <!-- Carregando carnês -->
        <nz-spin *ngIf="participanteId && carregandoCarnes"
          nzTip="Carregando carnês..." style="display:block;text-align:center;padding:60px" />

        <!-- Conteúdo carregado -->
        <ng-container *ngIf="participanteId && !carregandoCarnes">
          ...
          <div class="perfil">
            ...
            <div class="perfil-stats">
              <div>
                <div class="stat-val">{{ carnes.length }}</div>
                <div class="stat-lbl">carnê(s)</div>
              </div>
              <div>
                <div class="stat-val verde">{{ carnesQuitados }}</div>
                <div class="stat-lbl">quitado(s)</div>
              </div>
              <div>
                <div class="stat-val" style="color:#fa8c16">{{ carnes.length - carnesQuitados }}</div>
          ...
          <!-- Sem carnês -->
          <nz-empty *ngIf="carnes.length === 0"
            nzNotFoundContent="Este participante não tem carnês nesta campanha" />

          <!-- Com carnês -->
          <div *ngIf="carnes.length > 0" style="display:flex; flex-direction:column; flex:1; min-height:0; overflow:hidden">

            <!-- Desktop: chips -->
            <p class="carnes-label">Carnês — clique para ver parcelas</p>
            <div class="carnes-chips">
              <div *ngFor="let c of carnes"
                   class="carne-chip"
                   [class.ativo]="c.id === carneId && !c.quitado"
                   [class.quitado]="c.quitado"
                   [nz-tooltip]="c.quitado ? 'Todas as parcelas pagas' : pagas(c) + ' de ' + c.parcelas.length + ' pagas'"
                   (click)="selecionarCarne(c)">
                <div class="chip-num" [class.quitado]="c.quitado">Nº {{ c.numeroSorte }}</div>
                <div class="chip-prog">
            ...
            <div class="section-label">Carnês — toque para ver parcelas</div>
            <div class="carnes-chips-mobile">
              <div *ngFor="let c of carnes"
                ...
                <div class="cc-mobile-num" [class.quitado]="c.quitado">
                  <span *ngIf="!c.quitado">Nº {{ c.numeroSorte }}</span>
                  <span *ngIf="c.quitado">✓ {{ c.numeroSorte }}</span>
                </div>
            ...
            <nz-divider *ngIf="carneSelecionado" style="flex-shrink:0;margin:12px 0" />

            <!-- Parcelas do carnê selecionado -->
            <ng-container *ngIf="carneSelecionado">

              <!-- Desktop: header -->
              <div class="carne-header">
                <span class="carne-header-title">
                  Carnê Nº {{ carneSelecionado.numeroSorte }} — Parcelas
                </span>
              ...
              <div class="section-label">
                Carnê Nº {{ carneSelecionado.numeroSorte }}
              ...
              <div style="flex:1; min-height:0; overflow-y:auto; padding-right:4px">
                ...
                <div *ngFor="let p of carneSelecionado.parcelas"
                     class="parcela-item"
            ...
            <!-- Nenhum carnê selecionado ainda -->
            <div *ngIf="!carneSelecionado" class="empty-center" style="min-height:160px">
              <span style="font-size:32px;color:#ddd" nz-icon nzType="book" nzTheme="outline"></span>
              <p style="color:#ccc;margin-top:8px;font-size:13px">Selecione um carnê acima</p>
            </div>
          </div>
...
export class PagamentosComponent implements OnInit {
  ...
  carnes: any[] = [];
  carregandoCarnes = false;
  carneId: string | null = null;
  ...
  carregarCarnes() {
    ...
    this.carregandoCarnes = true;
    this.api.get<any[]>('carnes/participante', { campanhaId, participanteId: this.participanteId! }).subscribe({
      next: (data) => {
        this.carnes = data;
        this.carregandoCarnes = false;
        const pendente = data.find(c => !c.quitado);
        this.carneId = (pendente ?? data[0])?.id ?? null;
      },
      error: () => { this.carregandoCarnes = false; },
    });
  }

  selecionarCarne(carne: any) { this.carneId = carne.id; }

  get carneSelecionado(): any | null {
    return this.carnes.find(c => c.id === this.carneId) ?? null;
  }
  ...
  get valorParcela(): number {
    const c = this.campanhaAtiva.campanha();
    if (!c) return 0;
    return Number(c.valorCarne) / c.numParcelas;
  }

  get carnesQuitados(): number {
    return this.carnes.filter(c => c.quitado).length;
  }
  ...
  get modalTitulo(): string {
    if (!this.parcelaSelecionada || !this.carneSelecionado) return 'Registrar Pagamento';
    return `Parcela ${this.pad(this.parcelaSelecionada.numero)} — Carnê Nº ${this.carneSelecionado.numeroSorte}`;
  }

  pagas(carne: any): number {
    return carne.parcelas.filter((p: any) => p.status === 'paga').length;
  }

  abrirPagamento(parcela: any) {
  ...
  confirmarPagamento() {
    ...
    this.api.post<any>('pagamentos', body).subscribe({
      next: (res) => {
